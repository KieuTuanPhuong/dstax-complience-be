...
```bash
pyenv virtualenv compliance
pyenv shell compliance
...
CREATE ROLE compliance WITH LOGIN PASSWORD 'password';
ALTER DATABASE compliance OWNER TO compliance;
```
...
```bash
ENVIRONMENT=local celery -A compliance.celery_tasks worker -l info -Q default
ENVIRONMENT=local celery -A compliance.celery_tasks beat -l info
```
...
ENVIRONMENT=local celery -A compliance.celery_tasks flower
```
